...
const INDIA_LOCATIONS = ["India","Bangalore","Hyderabad","Chennai","Mumbai","Pune","Delhi NCR","Noida","Gurgaon","Kochi","Ahmedabad","Kolkata","Coimbatore","Jaipur"];

async function fetchExternalCandidates(skills, location) {
  try {
    const res = await fetch("http://localhost:3001/api/external-search/run", { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ skills, location, requiredSkills:skills }) });
    if (!res.ok) throw new Error("bad response");
    const data = await res.json();
    return data.candidates?.length ? data.candidates : null;
  } catch { return null; }
}

function generateMockExternal(skills, location) {
  const names = ["Arjun Pillai","Sneha Reddy","Vikram Das","Pooja Shetty","Karthik Menon","Divya Nair"];
  const roleMap = { "AI":["AI Engineer","ML Engineer"], "Python":["Python Developer","Backend Engineer"], "Embedded":["Embedded Engineer","Firmware Developer"], "PCB":["PCB Designer","Hardware Engineer"], "CAD":["CAD Engineer","Mechanical Designer"], "Web":["Full Stack Developer","Frontend Developer"] };
  const matchedKey = Object.keys(roleMap).find(k => skills.some(s=>s.toLowerCase().includes(k.toLowerCase())));
  const roleList = roleMap[matchedKey] || ["Software Engineer","Technical Engineer"];
  return names.map((name,i) => {
    const exp = Math.floor(Math.random()*6)+2;
    const score = Math.floor(Math.random()*25)+65;
    return { id:`mock_${i}`, name, role:roleList[i%roleList.length], experience:exp, location: i%3===0?location:INDIA_LOCATIONS[Math.floor(Math.random()*INDIA_LOCATIONS.length)], skills:[...skills.slice(0,3),"Python","Communication"].slice(0,5), availability:"Open to Work", status:"external", source:i%2===0?"LinkedIn":"Naukri.com", profileUrl:i%2===0?`https://linkedin.com/in/${name.toLowerCase().replace(/\s+/g,'-')}`:`https://naukri.com/profile/${name.toLowerCase().replace(/\s+/g,'-')}`, company:"Currently Seeking", isExternal:true, avatar:name.split(" ").map(w=>w[0]).join("").toUpperCase(), score, skillMatch:score-5, experienceScore:Math.min(100,exp*12), availabilityScore:100, explanation:`${name} has ${exp} years experience. Open to work in ${location}.` };
  });
}
...
function ResourceFinder({ shortlisted, setShortlisted }) {
  ...
  const handleSearch = async () => {
    if (!jd.trim()) return;
    setLoading(true); setResults(null); setExternalResults(null); setShowPhase2(false); setStatus("AI analyzing requirement...");
    ...
    if ((rankings[0]?.score || 0) < 75) setShowPhase2(true);
  };

  const handlePhase2Search = async () => {
    setPhase2Loading(true); setExternalResults(null);
    setPhase2Status("🔍 Searching LinkedIn + Naukri + Indeed...");
    const live = await fetchExternalCandidates(extractedSkills, location);
    const final = live || generateMockExternal(extractedSkills, location);
    setExternalResults(final);
    setPhase2Status(`✅ Found ${final.length} external candidates for ${location}`);
  ...
  return (
    <div style={{ display:"flex", height:"100%", overflow:"hidden" }}>
      <div style={{ width:340, borderRight:`1px solid ${t.border}`, padding:20, display:"flex", flexDirection:"column", gap:14, overflowY:"auto" }}>
        ...
        <textarea value={jd} onChange={e=>setJd(e.target.value)} placeholder={"Describe the task requirement...\n\ne.g. Need someone for AI-based PCB routing. Python and AI skills required."} style={{ width:"100%", height:160, padding:12, boxSizing:"border-box", background:t.surfaceAlt, border:`1px solid ${t.border}`, borderRadius:9, color:t.text, fontSize:12, resize:"vertical", fontFamily:"inherit", lineHeight:1.6 }} />
        <div>
          <div style={{ fontSize:10, color:t.dim, marginBottom:6, textTransform:"uppercase" }}>Quick Templates</div>
          <div style={{ display:"flex", flexWrap:"wrap", gap:5 }}>
            {[["PCB AI","Need someone for AI circuit creation and PCB routing. AI and Python skills required."],["Embedded","Need engineer for firmware flashing and embedded development. C and Embedded."],["Web + Python","Web development and Python scripting task. Looking for available team member."],["CAD","CAD model design and fabrication. Need someone with CAD skills immediately."],["CV / AI","Computer vision or video/audio analysis. Python and AI/ML skills needed."],["Content","Poster and ad creation task. Photography and editing skills needed."]].map(([label,text]) => (
        ...
        <div>
          <div style={{ fontSize:10, color:t.dim, marginBottom:6, textTransform:"uppercase" }}>Filter by Role</div>
          <div style={{ display:"flex", flexWrap:"wrap", gap:5 }}>
            {roles.map(role => <button key={role} onClick={()=>setFilterRole(role)} style={{ padding:"4px 9px", borderRadius:5, border:`1px solid ${filterRole===role?t.accent:t.border}`, background:filterRole===role?t.accentGlow:t.surfaceAlt, color:filterRole===role?t.accent:t.dim, fontSize:10, cursor:"pointer", fontWeight:filterRole===role?600:400 }}>{role}</button>)}
          </div>
        </div>
        ...
        {(showPhase2 || externalResults) && (
          <div style={{ padding:14, borderRadius:10, background:"rgba(139,92,246,0.08)", border:`1px solid ${t.purple}44` }}>
            ...
            {showPhase2 && !externalResults && <div style={{ fontSize:11, color:t.dim, marginBottom:10 }}>Best internal match below 75%. Search Naukri + LinkedIn across India?</div>}
            ...
            <button onClick={handlePhase2Search} disabled={phase2Loading} style={{ width:"100%", padding:9, borderRadius:8, border:"none", cursor:phase2Loading?"not-allowed":"pointer", background:phase2Loading?t.muted:`linear-gradient(135deg,${t.purple},#EC4899)`, color:"#fff", fontSize:12, fontWeight:700 }}>
              {phase2Loading ? "🔍 Searching..." : "🌐 Search External Candidates"}
            </button>
        ...
        {!showPhase2 && results && !externalResults && (
          <button onClick={()=>setShowPhase2(true)} style={{ padding:9, borderRadius:8, border:`1px solid ${t.purple}44`, cursor:"pointer", background:t.purple+"15", color:t.purple, fontSize:11, fontWeight:600 }}>🌐 Also search external candidates</button>
        )}
        ...
        {results && (
          <>
            <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:14, padding:"10px 14px", borderRadius:9, background:t.accentGlow, border:`1px solid ${t.accent}44` }}>
              ...
              <span style={{ fontSize:11, color:t.dim, marginLeft:4 }}>{results.summary}</span>
              <span style={{ marginLeft:"auto", fontSize:13, fontWeight:700, color:results.rankings[0]?.score>=75?t.success:t.warning }}>Best: {results.rankings[0]?.score}% {results.rankings[0]?.score<75?"⚠️ Low":"✅"}</span>
            </div>
            <div style={{ display:"flex", flexDirection:"column", gap:10, marginBottom:externalResults?24:0 }}>
              {results.rankings.map((r,idx) => {
                const m = r.member; const myTasks = getMemberTasks(m.id); const isShortlisted = shortlisted.find(s=>s.id===m.id);
                return (
                  <div key={m.id} style={{ background:t.surface, border:`1px solid ${idx===0?t.accent+"66":t.border}`, borderRadius:11, padding:16 }}>
                    ...
                    <div style={{ marginTop:10, display:"flex", flexWrap:"wrap", gap:4 }}>{m.skills.map(s=><SkillTag key={s} skill={s} matched={extractedSkills.some(es=>es.toLowerCase()===s.toLowerCase())}/>)}</div>
                    ...
                    {myTasks.filter(tk=>tk.status==="In Progress").length>0 && <div style={{ marginTop:8, fontSize:10, color:t.warning }}>Active: {myTasks.filter(tk=>tk.status==="In Progress").map(tk=>tk.name).join(", ")}</div>}
              ...
              <>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                  {externalResults.map((c,idx) => {
                    ...
                    return (
                      <div key={c.id} style={{ background:t.surface, border:`1px solid ${idx===0?t.purple+"66":t.border}`, borderRadius:11, padding:16, borderTop:`3px solid ${srcColor}` }}>
                        ...
                        <div style={{ marginTop:10, display:"flex", flexWrap:"wrap", gap:4 }}>{c.skills.map(s=><SkillTag key={s} skill={s} matched={extractedSkills.some(es=>es.toLowerCase()===s.toLowerCase())}/>)}</div>
                        ...
                        <div style={{ display:"flex", gap:7, marginTop:12 }}>
                          <button onClick={()=>toggleShortlist(c)} style={{ flex:1, padding:7, borderRadius:7, border:`1px solid ${isShortlisted?t.success+"44":t.purple+"44"}`, cursor:"pointer", background:isShortlisted?t.success+"22":t.purple+"15", color:isShortlisted?t.success:t.purple, fontSize:11, fontWeight:600 }}>{isShortlisted?"✓ Shortlisted":"★ Shortlist"}</button>
            ...
            {phase2Loading && <div style={{ textAlign:"center", padding:30, color:t.purple }}><div style={{ fontSize:24, marginBottom:8 }}>🌐</div><div style={{ fontSize:13, fontWeight:600 }}>Scraping live job sites...</div></div>}
          </>
        )}
      </div>
    </div>
